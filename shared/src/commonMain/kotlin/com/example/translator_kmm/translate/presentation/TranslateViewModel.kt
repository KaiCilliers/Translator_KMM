...
// this view model will be wrapped by both android and iOS
class TranslateViewModel(
    ...
    val state: CommonStateFlow<TranslateState> =
        combine(_state, historyDataSource.getHistory()) { state, historyItems ->
            if (state.history != historyItems) {
                state.copy(
                    history = historyItems.mapNotNull { item ->
                        UiHistoryItem(
